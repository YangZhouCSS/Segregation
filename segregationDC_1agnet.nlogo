...
to setup
 ...
 gis:set-world-envelope gis:envelope-of dc-dataset 
  ...
  [ if gis:property-value ? "SOC" = "RED" [ gis:set-drawing-color red  gis:fill ? 2.0]    
    if gis:property-value ? "SOC" = "BLUE" [ gis:set-drawing-color blue  gis:fill ? 2.0]
    if gis:property-value ? "SOC" = "UNOCCUPIED" [ gis:set-drawing-color grey  gis:fill ? 2.0]
    ]
  ...
  [ let center-point gis:location-of gis:centroid-of ?
    ask patch item 0 center-point item 1 center-point [
      ...
      set mycolor gis:property-value ? "SOC" 
...
to go
  ...
  update-colors
  tick
  if count turtles with [happy? = true] = count turtles [stop]
end
...
to export
  ...
  file-close 
  file-delete "data/result.txt"
  file-open "data/result.txt" 
  file-print "ID_ID,SOC\r\n"   ;;writing the header here. Headers include ID and polygon color.
  ...
  while [i <= 188]     ;;there are 188 polygons
    [ file-type i      ;;writing ID
      ...
      ask patches with [ID = i] [file-print mycolor]    ;;ask the corresponding patch to write down its color
      file-type "\r\n"
      set i i + 1
     ] 
...
@#$#@#$#@
## WHAT IS IT?
This is a segregation model built using the map of Wahington DC. The form of data being used is vector data. This model is inspired by the Schelling segregation model.
...
You can export more data in the output file. Copy the information from the source data to patches and add some lines to ask patches to print them as well.
